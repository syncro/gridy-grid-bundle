...
class Renderer {
    ...
    prepareTemplate(tpl) {
        if (this.tplFmt && this.tplFmt == 'handlebars') {
            let html = this.jquery(tpl.innerHTML).prop('outerHTML');
...
class GridyFilter extends HTMLElement {
...
class GridyPager extends HTMLElement {
    ...
    renderPageButton(num, backPaged, sort) {
        let context = Object.assign({ num }, this);
        let path = num === this.curPageNum ? this.itemActiveTplPath : this.itemTplPath;
        let templateId = num === this.curPageNum ? 'pagerActiveItemTpl' : 'pagerItemTpl';
        this.renderer.mountTemplate(path, templateId, this, context).then((templateEl) => {
            ...
            this.querySelector('ul').appendChild(button);
    ...
    renderButtons(backPaged, sort) {
        let context = this;
        Promise.all([
            this.renderer.mountTemplate(this.bodyTplPath, 'pagerBodyTpl', this, context),
...
class DataSourceLocal extends EventTarget {
    ...
    loadData(data, params) {
        ...
        params = params || {};
        let dataRows = this.dataPath ? jsonPath(data, this.dataPath)[0] : data;
        ...
        this.dataFiltered = this._data.filter((dataItem) => {
            for (let field of this.fields) {
                if (typeof field == 'string') {
                    ...
                    let result = jsonPath(dataItem, field.path)[0];
                    if (result !== null) {
...
class DataSourceAjax extends DataSourceLocal {
...
class GridyDataSource extends HTMLElement {
...
class GridyTable extends HTMLElement {
    ...
    renderHeaderContents() {
        ...
        let headEl = this.tableEl.querySelector('thead');
        if (headEl != null) {
            headEl = this.headEl;
        } else {
    ...
    renderHeaders() {
        let context = this;
        this.renderer.mountTemplate(this.headTplPath, 'tableHeadTpl', this, context).then((templateEl) => {
            this.headTpl = templateEl;
    ...
    renderData(event) {
        ...
        this.renderer.mountTemplate(this.bodyTplPath, 'tableBodyTpl', this, context).then((templateEl) => {
            this.bodyTpl = templateEl;
...
class GridyTableInfo extends HTMLElement {
    ...
    renderData(loadByPage) {
        ...
        this.renderer.mountTemplate(path, id, this).then((templateEl) => {
            ...
            let map = this.buildContext();

            if (this.lang) {
                this.renderer.localizeEl(el, this.lang).then((translatedEl) => {
                    this.innerHTML = '';
...
class GridySpinner extends HTMLElement {
...
class GridyDataField extends HTMLElement {


}

class GridyGrid extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (!this.dataSource) {
            let dataSourceEl = this.querySelector('gridy-data-source');
        ...
        this.filters = [];
        let filters = document.querySelectorAll('gridy-filter');
        ...
        let pager = this.querySelector('gridy-pager');
        if (pager !== null) {
        ...
        let table = this.querySelector('gridy-table');
        if (table !== null) {
        ...
        this.infos = [];
        let infos = this.querySelectorAll('gridy-table-info');
        ...
        let spinner = this.querySelector('gridy-spinner');
        if (spinner !== null) {
...
export { GridyGrid, GridySpinner, GridyTable, GridyTableInfo, GridyPager, GridyFilter, GridyDataSource, GridyDataField, DataSourceLocal, DataSourceAjax, Renderer, EventBus, XmlHttpClient, HgTemplateEngine, BaseTemplateEngine };
